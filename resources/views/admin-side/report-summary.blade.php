            ...
            <table class="min-w-full divide-y divide-gray-200 border-collapse border border-gray-400">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($instructors as $instructor)
                        <tr>
                            <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-800 text-center">{{ $loop->iteration }}</td>
                            <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-800 text-center">{{ $instructor->lastname }}, {{ $instructor->firstname }} {{ strtoupper(substr($instructor->middlename, 0, 1)) }}.</td>
                            <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-800 text-center">{{ $instructor->sex }}</td>
                            <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-800 text-center">{{ $instructor->instructor_id }}</td>
                            <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-800 text-center">{{ $instructor->rating ?? 'N/A' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
